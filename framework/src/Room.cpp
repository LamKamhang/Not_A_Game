...
Room::Room(const std::vector<float> &_wall, const std::vector<float> &_floor, 
        ...
        const std::vector<glm::vec3> &light_pos, const Light &light):
        wall_vertices(_wall), floor_vertices(_floor), 
        wall_material(wall_material), floor_material(floor_material), ceil_material(ceil_material),
        light_pos(light_pos), light(light), 
        directory("Resource/Texture")
{
    bindVAO();
}
...
{
    // set up light
    setLight(materialShader, "light", light);
    setLightPos(materialShader, "light_pos");
...
inline void Room::setLight(Shader &shader, const std::string &name, Light &value)
{
...
inline void Room::setLightPos(Shader &shader, const std::string &name)
{
    std::string num;
    for (int i = 0; i < light_pos.size(); ++i)
    {
        num = std::to_string(i);
        shader.setVec3(name + "[" + num + "]", light_pos[i]);
    }
}
...
inline void Room::bindVAO()
{
	...
	glBindVertexArray(floor_VAO);
		...
		glBindBuffer(GL_ARRAY_BUFFER, floor_VBO);
			glBufferData(GL_ARRAY_BUFFER, _floor.size() * sizeof(float), &_floor[0], GL_STATIC_DRAW);
	...
	glBindVertexArray(wall_VAO);
		...
		glBindBuffer(GL_ARRAY_BUFFER, wall_VBO);
			glBufferData(GL_ARRAY_BUFFER, _wall.size() * sizeof(float), &_wall[0], GL_STATIC_DRAW);
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), (GLvoid *)(0));
			glEnableVertexAttribArray(0);
			glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 8*sizeof(GLfloat), (GLvoid *)(3*sizeof(GLfloat)));
			glEnableVertexAttribArray(1);
...
inline void Room::initRoom(Camera &camera)
{
    ...
    light_pos = GetFirstFloorDefaultLightPos();
...
std::vector<float> GetFirstFloorDefaultWall(Camera &camera)
{
    ...
    std::vector<Rule> rules{
       ...
       Rule(_back, 3, wall_height,  32+1.5, cy, 60-wall_width),//8
       ...
       Rule(_back, wall_width, wall_height,  40-wall_width, cy, 55),//16
       Rule(_back, wall_width, wall_height,  40-wall_width, cy, 52),//17
       ...
       Rule(_back, wall_width, wall_height,  65+wall_width, cy, 30),//22
       ...
       Rule(_left, 15, wall_height,  40, cy, 6),//12
...
Light GetFirstFloorDefaultLight()
{
    return Light(glm::vec3(1.0), 0.1, 0.5, 0.4);
}
