...
public:
    ...
    void updateParticlesFireworks(GLfloat deltaTime,const glm::vec3 &CameraPosition,float height,const glm::vec3 &bulletPos,bool IsAttacking)
    {
        ...
        for(int i = 0;i < newparticles; i++){
            ...
            // 烟花
            p.speed = p.speedrate * glm::vec3(glm::sin(theta)*glm::sin(p.angle),glm::cos(theta),glm::sin(theta)*glm::cos(p.angle));
        }
    ...
    void updateParticlesFountain(GLfloat deltaTime,const glm::vec3 &CameraPosition,const glm::vec3 &startPos)
    {
        ...
        for(int i = 0;i < newparticles; i++){
            ...
            p.cameradistance = glm::length( p.pos - CameraPosition );
            // 喷泉
            p.speed = p.speedrate * glm::normalize(glm::vec3(glm::sin(p.angle),3.0f,glm::cos(p.angle)));
        }
    ...
    void updateParticlesFlame(bool IsAttacking,float deltaTime,const glm::vec3 &BulletPos,const glm::vec3 &BulletDir,const glm::vec3 &CameraPos)
    {
        ...
        for(int i = 0;i < newparticles; i++){
            ...
            p.speed = p.speedrate * ( alpha * (glm::cos(p.angle) * n + glm::sin(p.angle) * m) + glm::normalize(BulletDir));
        }
    ...
    void updateParticlesOpenFire(bool IsAttacking,float deltaTime,float curTime,const glm::vec3 &ganPos,const glm::vec3 &ganDir,const glm::vec3 &CameraPos)
    {
        ...
        for(int i = 0;i < newparticles; i++){
            ...
            p.speed = p.speedrate * ( alpha * (glm::cos(p.angle) * n + glm::sin(p.angle) * m) + glm::normalize(ganDir));
        }
